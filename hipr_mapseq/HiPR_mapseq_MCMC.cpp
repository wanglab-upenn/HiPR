...
// reads DMA-MaPseq collapsed reads
// read<N>@@count@@length start0 end0 mismatch_positionCSV:mismatch_nucsCSV
struct Read {
	std::string id;
	int start;
	int end;
	int nocc;
	vector<int> mism_pos; /* array with mismatched positions 1-based */
	int num_mism; /* number of mismatches */
        std::string mism_list;
	int count;
	int len;
	Read() {}
	// read a *.reads row
	Read(std::string &line) {
		...
		if (!junk.empty()) {
			...
			is >> mism_list;
                        ...
                        {
                           std::string sp; 
                           if (!getline( misms, sp, ',' )) break;
                           //cout << sp << endl;
                           int p = atoi(sp.c_str());
                           if (p!=-1) 
                           {
                             mism_pos.push_back( p );
                             ++mism_cnt;
                           }
                        }
                        //cout << "mism_cnt=" << mism_cnt << endl;
                        //cout << "mism_pos size=" << mism_pos.size() << endl;
                        num_mism = mism_cnt;
                        nocc = 3;
		}
	}
...
double compute_LLdmsmapseq (std::string structure, std::string sequence, std::vector<vector<int> > &reads, std::vector<double> &u, std::vector<double> &v, double r, int lcut, int rcut, int model, vector<vector<double> > ll_lookup);

double compute_LLdmsmapseqProd (std::string structure, std::string sequence, std::vector<vector<int> > &reads, vector<double> &u, vector<double> &v, double r, int lcut, int rcut, int model, std::vector< vector<double> > ll_lookup, std::vector<int> &readMisms, std::vector<int> &readThroughs);
...
int main(int argc, char **argv) {
	...
	int model = 2;
        int MAX_MISM = 1; // max # of mismatches per read
	...
	for (int i=1; i<argc; i++) {
		...
		if (arg.substr(0,1).compare("-")!=0) {
			...
			likelihoodOnly = true;
                ...
                else if (arg.compare("-max_mism")==0) {
                       if (argc < i + 2) {
                          cerr << usage;
                          return(-1);
                       }
                       MAX_MISM = atoi(argv[++i]);
                }
		else {
...
//	cout << fixed;
	doMCMC(structure, seq, reads, u, v, r, lcut, rcut, model, mcConstant, num_iters, doStructure, doDigestion, doFragmentation, weightMFE, likelihoodOnly, doLog, log_fn, out_fn, MAX_MISM);
...
bool checkDigestionParameters(vector<double> &u, vector<double> &v, double r) {
	...
	for (int i=0; i<u.size()-2; i++) { // only force vA>uA and vC>uC
		if (v[i] < u[i]) {
			return false;
		}
	}
...
//	p[ind] += change;
	...
	for(int i = 0; i < p.size(); i++) {
		if (p[i] != -1) {
                        double step = 0.1 * p[i];	
			p[i] += drand(-step, step); // random step in [-0.01, 0.01]
...
// Calculates the value of the log-likelihood function for the given structure, digestion parameters, and read data
double compute_LLdmsmapseq (std::string structure, std::string seq, std::vector<vector<int> > &reads, vector<double> &u, vector<double> &v, double r, int lcut, int rcut, int model, std::vector< vector<double> > ll_lookup) {
	// variable declaration
	
	double totalLL = 0; // normalization factor
	double ll = 0; // log likelihood (unnormalized)
	double extend = 0;
	int start, stop;
	...
	double sum = 0;
        ...
        int MAX_mism_cnt = reads[0].size() - 3 - 1;  
        bool useAConly = false; /* plant mismatches at A or C only during Z computation */ 

        /* adjust min and max read length to account for 2nt trimming */
        //lcut = lcut - 2;
        //rcut = rcut - 2;       
 
	for (int i=0; i < (structure_length - lcut + 1); ++i)
        {
          int j_min = i + lcut - 1; /* end of read with min len */
          int j_max = i + rcut - 1; /* end of read with max len */
  	  ...
  	  for (int k=i; k <= j_min; ++k)
          ...
          base_ll = ll; /* log P(shortest read [i,j_min], no mutation) */ 
	  ...
	  sum += exp(ll);
          ...
          if (MAX_mism_cnt > 0)
          {
	    /* 1-mismatch reads contributions */
	    for (int k = i; k <= j_min; ++k)
	    {
              if ( !useAConly ||
                   (seq[k]=='A' || seq[k]=='C') )
              {
		ll = base_ll - nodigestion_probs[k] + digestion_probs[k];    
		sum += exp(ll);
		++cnt;
	      }
	    }
          ...
          if (MAX_mism_cnt > 1)
          {
          /* m=2 mismatches for the shortest read */
          for (int k1=i; k1 <= j_min-1; ++k1)
            for (int k2=k1+1; k2 <= j_min; ++k2)
            {
              if ( !useAConly ||
                   (seq[k1]=='A' || seq[k1]=='C') &&
                   (seq[k2]=='A' || seq[k2]=='C') )
              {
		ll = base_ll - nodigestion_probs[k1] - nodigestion_probs[k2] + digestion_probs[k1] + digestion_probs[k2];
		sum += exp(ll);
		++cnt;
	      }
            }
	  ...
	  for (int j=j_min+1; j <= j_max; ++j)
	    ...
	    if (model==2)
	      ...
	      /* add read without mismatch */
	      sum += exp(ll);
              cnt++;

              if (MAX_mism_cnt > 0)
              {
              /* add reads with mismatch at the i,i+1,..,jth positions */
              for (int k1 = i; k1 <= j ; ++k1)
              {
                ...
                {
                  sum += exp(ll - nodigestion_probs[k1] + digestion_probs[k1]);
                  cnt++;
                }
              }
              }
		...
		for (int k1=i; k1 <= j-1; ++k1)
		  for (int k2=k1+1; k2 <= j; ++k2)
		  {
		    ...
		    {
		      sum += exp( ll - nodigestion_probs[k1] - nodigestion_probs[k2] + digestion_probs[k1] + digestion_probs[k2] );
		      ++cnt;
		    }
		  }
              }

              ll_prev = ll; /* read i,j no mismatches */
              //cout << "ll[" << i << "][" << j << "]=" << ll << endl;
	    }
	  } // end j
	} // end i
	...
	// Go through the reads and keep a running total of the log-likelihood
        ...
        for (int i=0; i < nReads; ++i )
        {
           int readStart = reads[i][0]; /* 0-based */
           int readEnd = reads[i][1]; /* 0-based */
           int readCount = reads[i][2];
           int readNumMism = reads[i][3];
   
           /* skip reads without mismatches */
           if (readNumMism==0) continue;

           /* compute read likelihood */
	   double ll = 0; // read likelihood
           for (int k=readStart; k<=readEnd; ++k) {
             ll += nodigestion_probs[k];
	   }

           for (int k=1; k <= readNumMism; ++k)
           {
             int i_mism = reads[i][3+k]; /*0-based mismatch position within read */

             i_mism = readStart + i_mism; /* 0-based mismatch position within locus */
             /*cout << "read " << i << " " << readCount << ": " << readStart << " " << readEnd << "; mismatch:" << i_mism << endl;*/

             //ll -= nodigestion_probs[i_mism];
             //ll += digestion_probs[i_mism];
             ll += (digestion_probs[i_mism] - nodigestion_probs[i_mism]);
           }

           //ll = log( exp(ll) / sum ); /* normalize read likelihood */
           //cout << "Read " << i << ": " << readStart << " " << readEnd << " " << readCount << " " << readNumMism << " " << ll << endl;
           
           //totalLL += (double) readCount * ll;
         
           /* normalize read likelihood */
           //totalLL -= (double) readCount * logZ;
           totalLL += (double) readCount * (ll - logZ);
        }
...
double compute_LLdmsmapseqProd (std::string structure, std::string sequence, std::vector<vector<int> > &reads, vector<double> &u, vector<double> &v, double r, int lcut, int rcut, int model, std::vector< vector<double> > ll_lookup, std::vector<int> &readMisms, std::vector<int> &readThroughs) 
{
//(std::string structure, std::string seq, std::vector<vector<int> > &reads, vector<double> &u, vector<double> &v, double r, int lcut, int rcut, int model, std::vector< vector<double> > ll_lookup) {
	// variable declaration
	...
	// Go through the reads and keep a running total of the log-likelihood
        ...
        for (int i=0; i<structure_length-lcut+1; i++)
        {
            dataLL+=(readMisms[i]*digestion_probs[i]+readThroughs[i]*nodigestion_probs[i]);
            numReads += (readThroughs[i]+readMisms[i]);
        }	
...
void doMCMC(std::string structure, std::string seq, list<Read> &reads, vector<double> &u, vector<double> &v, double r, int lcut, int rcut, int model, double mcConstant, int num_iters, bool doStructure, bool doDigestion, bool doFragmentation, bool weightMFE, bool likelihoodOnly, bool doLog, std::string log_fn, std::string out_fn, int MAX_MISM) {
	...
	list<Read>::iterator it;
        ...
        vector<vector <int> > readMat(maxReads, vector<int>(3+MAX_MISM+1));

        vector<int> readMisms(structure_length); // readMisms[i]=#mismatches at position i
        vector<int> readThroughs(structure_length); // readThroughs[i]=#number of reads without mismatch at position i
...
//			cout << "skipping read with extend = " << extend << endl;
			continue;
                ...
                if ((it->end-it->start+1)>=lcut && (it->end-it->start+1)<=rcut && it->start>=0 && it->num_mism <= MAX_MISM)
                //if ((it->end-it->start+1)>=lcut && (it->end-it->start+1)<=rcut && it->start>=0 && it->num_mism <= MAX_MISM && it->num_mism>0) // filter out reads with no mismatches
                {
                    readMat[nReads][0]=it->start; /* 0-based */
                    readMat[nReads][1]=it->end; /* 0-based */
                    readMat[nReads][2]=it->count;
                    readMat[nReads][3]=it->num_mism;

                    
                    //readMat[nReads][0]=it->start+2; /* trim first 2 nts of read read due to non-template additions */
                    for (int j=it->start; j<=it->end; ++j)
                    {
                        readThroughs[j] += it->count;
                    }

                    for (int i=0; i < it->num_mism; ++i)
                    {   
                      int i_mism = it->mism_pos[i]; // relative position of the mismatch within the read
                      int k = it->start + it->mism_pos[i] - 1; // 0-based mismatch position with the structure [0, structure_length-1]
                      readMisms[k] += it->count;
                      readThroughs[k] -= it->count;  
                      readMat[nReads][4+i] = i_mism - 1; /* -1 to make 0-based */
                    }
                    ++nReads;
                }
        }
        ...
        for (int i=0; i<nReads; ++i)
        {
          cout << readMat[i][0] << " " << readMat[i][1] << " " << readMat[i][2] << " " << readMat[i][3];
          for (int j=0; j<readMat[i][3]; ++j)
             cout << " " << readMat[i][4+j];
          cout << endl;
        }
        exit(-1);
  ...
  if (doLog) {
	...
	bool validInitial = true; //checkDigestionParameters(u,v,r);
	if (!validInitial) {
...
// calculate initial likelihood
	...
	//ll = compute_LLdmsmapseq(structure, seq, readMat,per_site_u,per_site_v,r,lcut,rcut,model,ll_lookup);
	ll = compute_LLdmsmapseqProd(structure, seq, readMat, per_site_u,per_site_v,r,lcut,rcut,model,ll_lookup, readMisms, readThroughs);
...
//			cerr << "computing likelihood" << endl;
			...
			//ll = compute_LLdmsmapseq(newstruct,seq,readMat,per_site_u,per_site_v,r,lcut,rcut,model, ll_lookup);
	                ll = compute_LLdmsmapseqProd(structure, seq, readMat, per_site_u,per_site_v,r,lcut,rcut,model,ll_lookup, readMisms, readThroughs);
	                //llb = compute_LL(structure,reads,per_site_u,per_site_v,r,lcut,rcut,model);
...
//				cerr << "stuck in parameter random walk loop " << newu << " " << newv << " " << endl;
			...
			//ll = compute_LLdmsmapseq(structure, seq, readMat,per_site_newu,per_site_newv,r,lcut,rcut,model, ll_lookup);
